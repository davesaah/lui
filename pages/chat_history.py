...
from lib.database import get_all_sessions, get_messages, delete_session
...
for session_id, title, date in sessions:
    ...
    with col1:
        if st.button(f"{title} ({clean_date})", key=f"btn_{session_id}", use_container_width=True):
    ...
    with col2:
        ...
        with st.popover(":material/delete:"):
            ...
            if st.button("Confirm", key=f"del_{session_id}"):
                ...
                # If the deleted chat is the one currently loaded, reset the state
